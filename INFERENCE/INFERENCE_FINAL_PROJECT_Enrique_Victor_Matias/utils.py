...
def season_lr(df, season_selected, goals):
    """
    Perform linear regression on the selected season's data to determine the relationship between goals (scored or conceded) and points obtained.
...
def is_relegation(season, position):
    """
    Checks if a given position qualifies as a relegation position for the specified season.

    Parameters:
    - season (str): Represents the season in question. It's formatted as 'YYYY-YY' (e.g., '1995-96').
    - position (int): Denotes the finishing position of a team within the specified season.

    Returns:
    - relegation_status (bool): A boolean value representing whether the given position qualifies as a relegation position for the specified season.
    """

    # If the season is 1995-96 or 1996-97, positions 19 and lower are considered relegation positions
    if season in ['1995-96', '1996-97']:
        return position >= 19
    # For all other seasons, positions 18 and lower are considered relegation positions
    else:
        return position >= 18
...
def team_ucl_wins(team, df):
    last_matches = df.loc[df.groupby('SEASON')['DATE_TIME'].idxmax()]
    ...
    def determinar_ganador(row):
        if row['FTHG'] > row['FTAG']:
            ...
            if row['PENALTY_SHOOT_OUT'] == 1:
                return row['HOME_TEAM'] if row['FTHG'] == row['FTAG'] else row['AWAY_TEAM']
            else:
                return None
    
    last_matches['WINNER'] = last_matches.apply(determinar_ganador, axis=1)
    
    champions_count = last_matches['WINNER'].value_counts()
    print(f"The {team} has won the Champions League {champions_count[team]} times since 2017.")
    return(champions_count[team])